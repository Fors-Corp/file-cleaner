...
A home-directory scan is bound by directory I/O latency, and the Python
...
def scan(
    ...
    *,
    ...
    helper = helper_path()
    if helper is None or not walks:
        return None
    request = json.dumps(
        {
            "deny_home": deny_home,
            ...
            "walks": [
                {"base": w.base, "pattern": w.pattern, "kind": w.kind, "excludes": list(w.excludes)} for w in walks
            ],
        }
    )
    try:
